...
class pranjan77contigfilter4Test(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'pranjan77contigfilter4',
        ...
        for nameval in config.items('pranjan77contigfilter4'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = pranjan77contigfilter4(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_pranjan77contigfilter4_" + str(suffix)
